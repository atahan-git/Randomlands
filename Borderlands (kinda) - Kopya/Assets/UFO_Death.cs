...
public class UFO_Death : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (Hpin.hpi <= 0 && !isDeath)
        {
            ...
            int explosionCount = Random.Range(4, 7);

            for(int i = 0; i <= explosionCount; i++)
            {
                Invoke("InstantiateExplosion", Random.Range(0.2f, 5f));
            }
    ...
    {
        Instantiate(ExplosionPrefab, brokenShip.transform.position, brokenShip.transform.rotation);
    }
